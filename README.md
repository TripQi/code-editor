...
| `read_files` | `read_files(file_paths, encoding=None)` | 批量读取多个文件 | 绝对路径列表；encoding 为 None 时自动探测；返回列表，每项含 path/content/mimeType/isImage 或 path/error | 传相对路径；单文件场景（用 read_file 更高效） |

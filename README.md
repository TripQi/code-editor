...
| `read_file` | `read_file(file_path, offset=0, length=None, encoding=None)` | 流式读取文本/图片，自动探测/缓存编码（utf-8/gbk/gb2312），二进制返回提示；返回 content/mimeType/isImage/encoding/encodingConfidence | 绝对路径；offset<0 读尾；length 最大行数（超过 `CODE_EDIT_FILE_READ_LINE_LIMIT` 会截断）；乱码时显式传 `encoding` 覆盖；encoding 字段返回检测到的编码（如 "utf_8"、"gbk"） | 传 URL；非整数 offset/length；相对路径；超大文件无范围读取 |
| `read_files` | `read_files(file_paths, encoding=None)` | 批量读取多个文件 | 绝对路径列表；encoding 为 None 时自动探测；返回列表，每项含 path/content/mimeType/isImage/encoding/encodingConfidence 或 path/error | 传相对路径；单文件场景（用 read_file 更高效） |
...
| 名称 | 工具 | 功能 | 主要参数/说明 | 常见误用 |
| --- | --- | --- | --- | --- |
| `edit_block` | `edit_block(file_path, old_string, new_string, expected_replacements=1, expected_mtime=None, ignore_whitespace=False, normalize_escapes=False, encoding="utf-8")` | 精确替换，行尾规范化；>10MB 自动走流式精确匹配；返回结构化结果含修改位置 | 绝对路径且在允许目录内；old_string 为搜索文本，new_string 为替换文本；计数不符/空搜索/仅模糊命中会抛异常；大文件仅支持严格字面匹配；返回 `{status, message, file_path, replacements, locations}` | 期望计数错；空搜索；大文件下使用忽略空白/转义归一会报错 |
| `edit_blocks` | `edit_blocks(edits, error_policy="fail-fast", encoding="utf-8")` | 批量编辑：单次调用完成多个搜索替换 | `edits` 为编辑规格列表，每项含 `file_path`/`old_string`/`new_string`/`expected_replacements`(默认1)/`ignore_whitespace`(默认False)/`normalize_escapes`(默认False)；`error_policy`: fail-fast(遇错停止)/continue(跳过错误继续)/rollback(任一失败全部回滚)；同文件多次编辑按顺序执行；返回 `{status, message, total_edits, successful_edits, failed_edits, results}` | 大文件(>10MB)不支持；edits 为空列表；同文件编辑顺序依赖时未按正确顺序传入 |

#### edit_block 返回格式（v0.3.0+）

`edit_block` 和 `edit_blocks` 返回结构化数据，包含修改位置信息便于验证：
...
- 批量读取：`read_files(["/abs/a.py", "/abs/b.py", "/abs/c.py"])`。
- 批量编辑（同文件多处修改）：
```python
edit_blocks(edits=[
